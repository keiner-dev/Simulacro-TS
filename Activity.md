...
**Arquitectura del proyecto:** organización por carpetas y modularización con ES Modules (separación de capas: componentes, hooks, servicios/API, tipos).

1. **Sistema de tipos:** tipado estático consistente, interfaces y tipos avanzados (utility types, uniones, tipos condicionales según el caso).
2. **Genéricos:** aplicados a funciones o hooks reutilizables (ej. una capa de fetch tipada).
3. **Clases y objetos:** únicamente donde el problema lo amerite (no como requisito forzado).
4. **Capa de comunicación con la API:** `fetch` o Axios, con interceptores de request/response para inyectar el token y reaccionar a respuestas 401.
5. **Manejo estructurado de excepciones:** `try/catch/finally` diferenciando error de red, de validación y de autorización.
6. **Persistencia de sesión:** `localStorage` y `sessionStorage`, con criterio justificado sobre cuál usar.
7. **Enrutamiento protegido:** rutas restringidas por autenticación y por autorización basada en rol (RBAC).
8. **Composición de componentes:** props, estado local (`useState`) y efectos de sincronización con el backend (`useEffect`).
9. **Gestión de estado global:** Context API.
10. **Formularios controlados:** captura, validación y manejo de errores del servidor en el formulario.
11. **Renderizado declarativo:** JSX, renderizado condicional y listas dinámicas.
12. **Manejo de errores de renderizado:** Error Boundaries.
13. **Pruebas:** unitarias y de integración (Jest / React Testing Library).
